...
async fn try_gemini_summarize(chunks: &[String], title: Option<&str>) -> Option<PaperBrief> {
    ...
    let prompt = format!(
        r#"{}以下は学術論文のテキストです。
このテキストに実際に書かれている内容だけを使って、厳密なJSONのみを出力（前後に説明文なし）。
テキストに書かれていないことは絶対に書かないこと。推測・創作・補完は禁止。

キーは次の5つ（すべて日本語で記述）:
- "problem": このテキストが扱っている問題・研究ギャップ（1〜2文、テキストの内容のみ）
- "method": このテキストで採用している手法・アプローチ（1〜2文、テキストの内容のみ）
- "key_result": このテキストの主要な結果・発見（1〜2文、テキストの内容のみ）
- "limitation": このテキストに書かれた限界・課題（1文、テキストの内容のみ）
- "summary": 上記4つを1段落にまとめた総合要約（3〜5文、テキストの内容のみ）
...
/// トーン別の創作スタイル指示文を返す（物語の題材は常に論文内容）
fn tone_instruction(tone: &str) -> &'static str {
    match tone {
        "少年マンガ" => "【演出スタイル】熱血・友情・努力・成長の文体で描く。論文に登場する分子・酵素・細胞・現象を「若き研究者キャラ」や「ライバル」として擬人化し、台詞は短く感情的に。「諦めるな！」「これが俺たちの答えだ！」のような熱い表現を使う。ただし物語の核心は必ず論文の実際の発見とすること。",
        "SF" => "【演出スタイル】SF的・宇宙規模の視点で描く。論文の分子・タンパク質・遺伝子を異星人や機械生命体として擬人化し、冷静で知的、宇宙の神秘を感じさせるトーン。物語の核心は必ず論文の実際の発見とすること。",
        "社会派" => "【演出スタイル】社会問題・医療・患者の視点を前面に出す。論文の研究が社会・患者・医療現場に与えるインパクトを中心に据え、患者・医師・家族の感情を描く。物語の核心は必ず論文の実際の発見とすること。",
        "ホラー" => "【演出スタイル】論文が発見した異常・制御不能な現象の恐怖を描く。静かで不気味なトーン、日常の中に忍び込む科学的真実の恐怖。物語の核心は必ず論文の実際の発見とすること。",
        "恋愛" => "【演出スタイル】論文に登場する分子・タンパク質・細胞を擬人化し、「本来いるべき場所を離れた逸脱者」を禁じられた恋として描く。甘くせつなく、科学的真実を純愛の比喩で表現する。物語の核心は必ず論文の実際の発見とすること。",
        _ => "【演出スタイル】客観的かつ分かりやすいポップサイエンスのトーンで描く。専門知識がない読者にも伝わるよう平易に。物語の核心は必ず論文の実際の発見とすること。",
    }
}
...
async fn try_gemini_narrate(
    ...
    let prompt = format!(
        r#"あなたは学術論文を漫画化する専門編集者です。
以下の【論文要約】に書かれた実際の研究内容・発見・科学的事実を物語の中心に据えて、漫画企画書を日本語で作成してください。

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【絶対厳守ルール】
1. theme_line・characters・synopsis・panel_beats は全て、下記【論文要約】の実際の研究内容・発見・科学的事実を必ず中心に据えること
2. 論文に書かれていない架空の研究テーマ（例: NLP・量子コンピュータ・ブロックチェーン等）を勝手に作り出すことは禁止
3. 論文の主役である生体分子・酵素・細胞・現象・研究対象を漫画キャラクターとして擬人化すること
4. キャラクター名には論文の実際の研究対象名（例: 代謝酵素、核、クロマチン等）を反映させること
5. 指定トーンは「物語の語り口・演出スタイル」のみを規定し、物語の題材は必ず論文の実際の内容とすること
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

厳密なJSONのみ出力（前後に説明文・マークダウン記号なし）。以下の全キーを含めること:
- "theme_line": 論文の核心的発見を表す漫画テーマ一行（30文字以内、論文内容を必ず反映）
- "characters": 2〜4名の配列 [{{"name":"論文の研究対象に基づく名前","role":"役割","description":"論文での科学的役割を反映した説明20文字"}}]
- "synopsis": 3幕構成の粗筋（150〜300文字。論文の問題→手法→発見の流れで展開。第1幕→第2幕→第3幕と明示）
- "outline_markdown": Markdownの構成案（見出しと箇条書き、4〜8行）
- "panel_beats": ちょうど4要素の string 配列（起→承→転→結。各30〜60文字。論文の実際の発見に基づくこと）

【漫画トーン: {}】
{}

{}【論文要約（この内容を必ず物語の中核に使用すること）】
問題: {}
手法: {}
結果: {}
限界: {}
